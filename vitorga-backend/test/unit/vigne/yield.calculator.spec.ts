...
const makeVigne = (overrides: Partial<{
...
describe('YieldCalculator', () => {
  ...
  describe('getTauxPerte', () => {
    it('should return 0.30 for vines under 5 years old', () => {
      ...
      const expected = 5000 * (1 - 0.30) * 0.15 * 1;
    ...
    it('should return 0.10 for vines between 5 and 15 years old', () => {
      ...
      const expected = 5000 * (1 - 0.10) * 0.15 * 1;
      expect(result.rendementHa).toBeCloseTo(expected, 2);
    });
  ...
  describe('exposition bonus/malus', () => {
    it('should add 10% bonus for SUD exposition', () => {
      ...
      const vigne = makeVigne({ exposition: 'SUD', annee_plantation: CURRENT_YEAR - 20 });

      const result = calculator.calculate(vigne, makeParcelle(1));

    
      const base = 5000 * (1 - 0.05) * 0.15;
      expect(result.rendementHa).toBeCloseTo(base * 1.10, 2);
    });

    it('should add 5% bonus for EST exposition', () => {
      
      const vigne = makeVigne({ exposition: 'EST', annee_plantation: CURRENT_YEAR - 20 });
    ...
    it('should subtract 15% for NORD exposition', () => {
      const vigne = makeVigne({ exposition: 'NORD', annee_plantation: CURRENT_YEAR - 20 });
    ...
    it('should apply no bonus or malus for OUEST exposition', () => {
      ...
      const vigne = makeVigne({ exposition: 'OUEST', annee_plantation: CURRENT_YEAR - 20 });
  ...
  describe('rendement parcelle', () => {
    it('should multiply rendement_ha by surface_ha', () => {
      const vigne = makeVigne({ annee_plantation: CURRENT_YEAR - 20, exposition: 'OUEST' });
  ...
  describe('cas limites', () => {
    ...
    it('should sum yields for multiple vignes on same parcelle', () => {
      ...
      const vignes = [
        makeVigne({ densite: 5000, annee_plantation: CURRENT_YEAR - 20, exposition: 'SUD' }),
        makeVigne({ densite: 4500, annee_plantation: CURRENT_YEAR - 3, exposition: 'NORD' }),
      ];
      const parcelle = makeParcelle(3.5);
